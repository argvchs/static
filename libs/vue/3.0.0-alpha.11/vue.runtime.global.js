var Vue = (function (exports) {
  ...
  const getChildRoot = (vnode) => {
      ...
      const setRoot = (updatedRoot) => (rawChildren[index] = updatedRoot);
  ...
  function resolveProps(instance, rawProps) {
  ...
  */
  ...
  const PublicInstanceProxyHandlers = {
      get(target, key) {
          const { renderContext, data, propsProxy, accessCache, type, sink, appContext } = target;
  ...
  function defineAsyncComponent(source) {
      ...
      retryWhen = NO, maxRetries = 3, suspensible = true } = source;
