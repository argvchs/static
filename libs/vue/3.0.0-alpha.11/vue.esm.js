...
const errorMessages = {
    ...
    [41 /* X_V_SLOT_MIXED_SLOT_USAGE */]: `Mixed v-slot usage on both the component and nested <template>.` +
        `When there are multiple named slots, all slots should use <template> ` +
        `syntax to avoid scope ambiguity.`,
    [42 /* X_V_SLOT_DUPLICATE_SLOT_NAMES */]: `Duplicate slot names found. `,
    [43 /* X_V_SLOT_EXTRANEOUS_DEFAULT_SLOT_CHILDREN */]: `Extraneous children found when component already has explicitly named ` +
        `default slot. These children will be ignored.`,
    [44 /* X_V_SLOT_MISPLACED */]: `v-slot can only be used on components or <template> tags.`,
    [45 /* X_V_MODEL_NO_EXPRESSION */]: `v-model is missing expression.`,
    [46 /* X_V_MODEL_MALFORMED_EXPRESSION */]: `v-model value must be a valid JavaScript member expression.`,
    [47 /* X_V_MODEL_ON_SCOPE_VARIABLE */]: `v-model cannot be used on v-for or v-slot scope variables because they are not writable.`,
    [48 /* X_INVALID_EXPRESSION */]: `Error parsing JavaScript expression: `,
    [49 /* X_KEEP_ALIVE_INVALID_CHILDREN */]: `<KeepAlive> expects exactly one child component.`,
    // generic errors
    [50 /* X_PREFIX_ID_NOT_SUPPORTED */]: `"prefixIdentifiers" option is not supported in this build of compiler.`,
    [51 /* X_MODULE_MODE_NOT_SUPPORTED */]: `ES module mode is not supported in this build of compiler.`,
    [52 /* X_CACHE_HANDLER_NOT_SUPPORTED */]: `"cacheHandlers" option is only supported when the "prefixIdentifiers" option is enabled.`,
    [53 /* X_SCOPE_ID_NOT_SUPPORTED */]: `"scopeId" option is only supported in module mode.`
};
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (onComponentSlot) {
            ...
            context.onError(createCompilerError(41 /* X_V_SLOT_MIXED_SLOT_USAGE */, slotDir.loc));
            break;
        }
        ...
        if ((vIf = findDir(slotElement, 'if'))) {
            ...
            if (staticSlotName) {
                if (seenSlotNames.has(staticSlotName)) {
                    context.onError(createCompilerError(42 /* X_V_SLOT_DUPLICATE_SLOT_NAMES */, dirLoc));
                    continue;
                }
    ...
    if (!onComponentSlot) {
        if (!hasTemplateSlots) {
            ...
            if (hasNamedDefaultSlot) {
                context.onError(createCompilerError(43 /* X_V_SLOT_EXTRANEOUS_DEFAULT_SLOT_CHILDREN */, implicitDefaultChildren[0].loc));
            }
            else {
        ...
        if (node.children.length > 0) {
            if (vnodeTag === KEEP_ALIVE) {
                ...
                if ( node.children.length > 1) {
                    context.onError(createCompilerError(49 /* X_KEEP_ALIVE_INVALID_CHILDREN */, {
...
function buildProps(node, context, props = node.props, ssr = false) {
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if (prop.type === 6 /* ATTRIBUTE */) {
            ...
            if (name === 'slot') {
                if (!isComponent) {
                    context.onError(createCompilerError(44 /* X_V_SLOT_MISPLACED */, loc));
                }
                continue;
            }
...
const transformModel = (dir, node, context) => {
    const { exp, arg } = dir;
    if (!exp) {
        context.onError(createCompilerError(45 /* X_V_MODEL_NO_EXPRESSION */, dir.loc));
        return createTransformProps();
    }
    const expString = exp.type === 4 /* SIMPLE_EXPRESSION */ ? exp.content : exp.loc.source;
    if (!isMemberExpression(expString)) {
        context.onError(createCompilerError(46 /* X_V_MODEL_MALFORMED_EXPRESSION */, exp.loc));
        return createTransformProps();
    }
...
function baseCompile(template, options = {}) {
    ...
    /* istanbul ignore if */
    {
        if (options.prefixIdentifiers === true) {
            onError(createCompilerError(50 /* X_PREFIX_ID_NOT_SUPPORTED */));
        }
        else if (isModuleMode) {
            onError(createCompilerError(51 /* X_MODULE_MODE_NOT_SUPPORTED */));
        }
    }
    const prefixIdentifiers = !true ;
    if ( options.cacheHandlers) {
        onError(createCompilerError(52 /* X_CACHE_HANDLER_NOT_SUPPORTED */));
    }
    if (options.scopeId && !isModuleMode) {
        onError(createCompilerError(53 /* X_SCOPE_ID_NOT_SUPPORTED */));
    }
...
export { BaseTransition, Comment, Fragment, KeepAlive, Suspense, Teleport, Text, Transition, TransitionGroup, callWithAsyncErrorHandling, callWithErrorHandling, camelize, cloneVNode, compileToFunction as compile, computed$1 as computed, createApp, createBlock, createCommentVNode, createHydrationRenderer, createRenderer, createSSRApp, createSlots, createStaticVNode, createTextVNode, createVNode, defineAsyncComponent, defineComponent, effect, getCurrentInstance, h, handleError, hydrate, inject, isReactive, isReadonly, isRef, markNonReactive, markReadonly, mergeProps, nextTick, onActivated, onBeforeMount, onBeforeUnmount, onBeforeUpdate, onDeactivated, onErrorCaptured, onMounted, onRenderTracked, onRenderTriggered, onUnmounted, onUpdated, openBlock, popScopeId, provide, pushScopeId, reactive, readonly, ref, registerRuntimeCompiler, render, renderList, renderSlot, resolveComponent, resolveDirective, resolveDynamicComponent, resolveTransitionHooks, setBlockTracking, setTransitionHooks, shallowReactive, shallowRef, ssrContextKey, ssrUtils, toDisplayString, toHandlers, toRaw, toRefs, transformVNodeArgs, unref, useCSSModule, useSSRContext, useTransitionState, vModelCheckbox, vModelDynamic, vModelRadio, vModelSelect, vModelText, vShow, version, warn, watch, watchEffect, withCtx, withDirectives, withKeys, withModifiers, withScopeId };
