var Vue = (function (exports) {
  ...
  */
  ...
  const errorMessages = {
      ...
      [28 /* X_V_IF_KEY */]: `v-if branches must use compiler generated keys.`,
      [29 /* X_V_ELSE_NO_ADJACENT_IF */]: `v-else/v-else-if has no adjacent v-if.`,
      [30 /* X_V_FOR_NO_EXPRESSION */]: `v-for is missing expression.`,
      [31 /* X_V_FOR_MALFORMED_EXPRESSION */]: `v-for has invalid expression.`,
      [32 /* X_V_BIND_NO_EXPRESSION */]: `v-bind is missing expression.`,
      [33 /* X_V_ON_NO_EXPRESSION */]: `v-on is missing expression.`,
      [34 /* X_V_SLOT_UNEXPECTED_DIRECTIVE_ON_SLOT_OUTLET */]: `Unexpected custom directive on <slot> outlet.`,
      [35 /* X_V_SLOT_MIXED_SLOT_USAGE */]: `Mixed v-slot usage on both the component and nested <template>.` +
          `When there are multiple named slots, all slots should use <template> ` +
          `syntax to avoid scope ambiguity.`,
      [36 /* X_V_SLOT_DUPLICATE_SLOT_NAMES */]: `Duplicate slot names found. `,
      [37 /* X_V_SLOT_EXTRANEOUS_DEFAULT_SLOT_CHILDREN */]: `Extraneous children found when component already has explicitly named ` +
          `default slot. These children will be ignored.`,
      [38 /* X_V_SLOT_MISPLACED */]: `v-slot can only be used on components or <template> tags.`,
      [39 /* X_V_MODEL_NO_EXPRESSION */]: `v-model is missing expression.`,
      [40 /* X_V_MODEL_MALFORMED_EXPRESSION */]: `v-model value must be a valid JavaScript member expression.`,
      [41 /* X_V_MODEL_ON_SCOPE_VARIABLE */]: `v-model cannot be used on v-for or v-slot scope variables because they are not writable.`,
      [42 /* X_INVALID_EXPRESSION */]: `Error parsing JavaScript expression: `,
      [43 /* X_KEEP_ALIVE_INVALID_CHILDREN */]: `<KeepAlive> expects exactly one child component.`,
      // generic errors
      [44 /* X_PREFIX_ID_NOT_SUPPORTED */]: `"prefixIdentifiers" option is not supported in this build of compiler.`,
      [45 /* X_MODULE_MODE_NOT_SUPPORTED */]: `ES module mode is not supported in this build of compiler.`,
      [46 /* X_CACHE_HANDLER_NOT_SUPPORTED */]: `"cacheHandlers" option is only supported when the "prefixIdentifiers" option is enabled.`,
      [47 /* X_SCOPE_ID_NOT_SUPPORTED */]: `"scopeId" option is only supported in module mode.`
  };
  ...
  function validateBrowserExpression(node, context, asParams = false, asRawStatements = false) {
      ...
      try {
          ...
          context.onError(createCompilerError(42 /* X_INVALID_EXPRESSION */, node.loc, undefined, message));
      }
  }
  ...
  function processIf(node, dir, context, processCodegen) {
      ...
      if (userKey) {
          context.onError(createCompilerError(28 /* X_V_IF_KEY */, userKey.loc));
      }
      if (dir.name === 'if') {
  ...
  function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
      ...
      for (let i = 0; i < children.length; i++) {
          ...
          if (onComponentSlot) {
              ...
              context.onError(createCompilerError(35 /* X_V_SLOT_MIXED_SLOT_USAGE */, slotDir.loc));
              break;
          }
          ...
          if ((vIf = findDir(slotElement, 'if'))) {
              ...
              if (staticSlotName) {
                  if (seenSlotNames.has(staticSlotName)) {
                      context.onError(createCompilerError(36 /* X_V_SLOT_DUPLICATE_SLOT_NAMES */, dirLoc));
                      continue;
                  }
      ...
      if (!onComponentSlot) {
          if (!hasTemplateSlots) {
              ...
              if (hasNamedDefaultSlot) {
                  context.onError(createCompilerError(37 /* X_V_SLOT_EXTRANEOUS_DEFAULT_SLOT_CHILDREN */, implicitDefaultChildren[0].loc));
              }
              else {
          ...
          if (node.children.length > 0) {
              if (vnodeTag === KEEP_ALIVE) {
                  ...
                  if ( node.children.length > 1) {
                      context.onError(createCompilerError(43 /* X_KEEP_ALIVE_INVALID_CHILDREN */, {
  ...
  function buildProps(node, context, props = node.props, ssr = false) {
      ...
      for (let i = 0; i < props.length; i++) {
          ...
          if (prop.type === 6 /* ATTRIBUTE */) {
              ...
              if (name === 'slot') {
                  if (!isComponent) {
                      context.onError(createCompilerError(38 /* X_V_SLOT_MISPLACED */, loc));
                  }
                  continue;
              }
              ...
              if (!arg && (isBind || isOn)) {
                  ...
                  if (exp) {
                      ...
                      context.onError(createCompilerError(isBind
                          ? 32 /* X_V_BIND_NO_EXPRESSION */
                          : 33 /* X_V_ON_NO_EXPRESSION */, loc));
                  }
                  continue;
              }
  ...
  function processSlotOutlet(node, context) {
      ...
      if (propsWithoutName.length > 0) {
          ...
          if (directives.length) {
              context.onError(createCompilerError(34 /* X_V_SLOT_UNEXPECTED_DIRECTIVE_ON_SLOT_OUTLET */, directives[0].loc));
          }
      }
      return {
          slotName,
  ...
  const transformOn = (dir, node, context, augmentor) => {
      ...
      if (!dir.exp && !modifiers.length) {
          context.onError(createCompilerError(33 /* X_V_ON_NO_EXPRESSION */, loc));
      }
      let eventName;
  ...
  const transformBind = (dir, node, context) => {
      ...
      if (!exp ||
          ...
          context.onError(createCompilerError(32 /* X_V_BIND_NO_EXPRESSION */, loc));
          return {
  ...
  const transformModel = (dir, node, context) => {
      const { exp, arg } = dir;
      if (!exp) {
          context.onError(createCompilerError(39 /* X_V_MODEL_NO_EXPRESSION */, dir.loc));
          return createTransformProps();
      }
      const expString = exp.type === 4 /* SIMPLE_EXPRESSION */ ? exp.content : exp.loc.source;
      if (!isMemberExpression(expString)) {
          context.onError(createCompilerError(40 /* X_V_MODEL_MALFORMED_EXPRESSION */, exp.loc));
          return createTransformProps();
      }
  ...
  function baseCompile(template, options = {}) {
      ...
      /* istanbul ignore if */
      {
          if (options.prefixIdentifiers === true) {
              onError(createCompilerError(44 /* X_PREFIX_ID_NOT_SUPPORTED */));
          }
          else if (isModuleMode) {
              onError(createCompilerError(45 /* X_MODULE_MODE_NOT_SUPPORTED */));
          }
      }
      const prefixIdentifiers = !true ;
      if ( options.cacheHandlers) {
          onError(createCompilerError(46 /* X_CACHE_HANDLER_NOT_SUPPORTED */));
      }
      if (options.scopeId && !isModuleMode) {
          onError(createCompilerError(47 /* X_SCOPE_ID_NOT_SUPPORTED */));
      }
